...
import { ItinerarySchema, TripPreferencesSchema, UpdatesFeedSchema, } from '@travel-engine/shared';
...
function buildFallbackItinerary(preferences) {
    ...
    const days = dates.map((date, dayIndex) => {
        const baseActivities = [
            ...
            },
            {
                ...
                type: preferences.styles.includes('nature') ? 'nature' : 'culture',
                cost: '$',
                duration: '2h',
                location: preferences.destination,
            ...
            },
            {
                ...
                type: preferences.styles.includes('adventure') ? 'adventure' : 'leisure',
                cost: '$$',
                duration: '2h',
                location: preferences.destination,
                tags: ['highlight'],
            ...
            },
            {
                ...
                type: preferences.styles.includes('nightlife') ? 'leisure' : 'food',
                cost: '$$',
                duration: '2h',
                location: preferences.destination,
                tags: ['dinner'],
...
function buildFallbackUpdates(destination) {
...
export async function fetchLiveTravelUpdates(destination, travelDates) {
    if (!client) {
...
export async function* streamConciergeChat(message, history, tripContext) {
    if (!client) {
